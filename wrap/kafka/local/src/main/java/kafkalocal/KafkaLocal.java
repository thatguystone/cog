...
public class KafkaLocal {
	public static void main(String []args) throws Exception {
		if (args.length != 1) {
			System.out.println("Usage: kafkalocal <datadir>\n");
			System.exit(2);
		}
	...
	static void startZk(final String dataBase) throws Exception {
		Properties props = loadProps("/zk.properties");
		props.setProperty("dataDir", dataBase + "/zk");
	...
	static void startKafka(final String dataBase) throws IOException {
		Properties props = loadProps("/kafka.properties");
		props.setProperty("log.dirs", dataBase + "/kafka");
